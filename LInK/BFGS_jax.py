...
def wolfe_line_search_batch(f, x, p, c1=1e-4, c2=0.9, max_iter=10, tau=0.5):
    ...
    for _ in range(max_iter):
        ...
        conditions_met = np.logical_and(armijo_condition, curvature_condition).flatten()

        alpha[~conditions_met] *= tau
        converged = np.logical_or(converged, conditions_met)
...
def bfgs_update_batch(H_k, s_k, y_k):
    ...
    """
    B, d, _ = H_k.shape
    I = np.eye(d).reshape([1,d,d]).repeat(B, 0)
    
    rho_k = 1.0 / (y_k * s_k).sum(axis=1)
    term1 = s_k[:,:,None] * y_k[:,None]  # shape [B, d, d]
    term2 = y_k[:,:,None] * s_k[:,None]  # shape [B, d, d]

    H_kp1 = (I - term1 * rho_k[...,None,None]) @ H_k @ (I - term2 * rho_k[...,None,None]) + (s_k [:,:,None] * s_k[:,None]) * rho_k[...,None,None]
    return H_kp1
...
def Batch_BFGS(x0s, obj_fn, max_iter = 100, threshhold = 0.01, tol=1e-6, f_tol = 1e-5, tau=0.5, line_search_max_iter=10, progress= lambda x: x):
    ...
    while True:
        ...
        if np.linalg.norm(nabla,axis=-1).max()<=tol:
            break
        
        it += 1
        progress([it/max_iter,f.min()])
        ...
        p = -np.matmul(H,nabla[...,None]).squeeze() # search direction (Newton Method)
        ...
        x_new = x + (alpha[...,None] * p).reshape(x.shape)
